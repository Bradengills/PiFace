...
webcam = picamera.PiCamera(0)				# Get ready to start getting images from the webcam
webcam.set(picamera.cv.CV_CAP_PROP_FRAME_WIDTH, 320)		# I have found this to be about the highest-
webcam.set(picamera.cv.CV_CAP_PROP_FRAME_HEIGHT, 240)	# 	resolution you'll want to attempt on the pi

frontalface = picamera.CascadeClassifier("haarcascade_frontalface_alt2.xml")		# frontal face pattern detection
profileface = picamera.CascadeClassifier("haarcascade_profileface.xml")		# side face pattern detection
...
while True:
	...
	if not faceFound:
		if lastface == 0 or lastface == 1:
			...
			fface = frontalface.detectMultiScale(aframe,1.3,4,(picamera.cv.CV_HAAR_DO_CANNY_PRUNING + picamera.cv.CV_HAAR_FIND_BIGGEST_OBJECT + picamera.cv.CV_HAAR_DO_ROUGH_SEARCH),(60,60))
	...
	if not faceFound:				# if we didnt find a face yet...
		if lastface == 0 or lastface == 2:	# only attempt it if we didn't find a face last loop or if-
			...
			aframe = webcam.read()[1]
			pfacer = profileface.detectMultiScale(aframe,1.3,4,(picamera.cv.CV_HAAR_DO_CANNY_PRUNING + picamera.cv.CV_HAAR_FIND_BIGGEST_OBJECT + picamera.cv.CV_HAAR_DO_ROUGH_SEARCH),(80,80))
	...
	if not faceFound:				# a final attempt
		if lastface == 0 or lastface == 3:	# this is another profile face search, because OpenCV can only-
			...
			aframe = webcam.read()[1]
			aframe = webcam.read()[1]
			picamera.flip(aframe,1,aframe)	#	flip the image
			pfacel = profileface.detectMultiScale(aframe,1.3,4,(picamera.cv.CV_HAAR_DO_CANNY_PRUNING + picamera.cv.CV_HAAR_FIND_BIGGEST_OBJECT + picamera.cv.CV_HAAR_DO_ROUGH_SEARCH),(80,80))
			if pfacel != ():
				lastface = 3
				for f in pfacel:
					faceFound = True
